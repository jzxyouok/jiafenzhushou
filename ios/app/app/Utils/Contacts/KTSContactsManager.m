...
- (void)importContacts:(void (^)(NSArray *))contactsHandler
{
    ...
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"请添加通讯录访问权限" message:@"请添加通讯录访问权限 -> 隐私 -> 通讯录" delegate:nil cancelButtonTitle:@"取消" otherButtonTitles:@"好的", nil];
        [alert show];
        return;
    }
...
- (NSMutableArray *)extractContactsInDictionary:(NSMutableArray *)contactsList extractOptions:(KTSContactsManagerField)fields
{
    ...
    [contactsList enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        //Image
        if (KTSContactsManagerFieldOrganization & fields){
            person[@"organization"] = [self stringProperty:kABPersonOrganizationProperty fromContact:record];
        }
...
- (void)addContactName:(NSString *)phone completion:(void (^)(BOOL))added
{
    CFErrorRef *error = nil;
    ABAddressBookRef addressBook = ABAddressBookCreateWithOptions(nil, error);
    ...
    ABRecordSetValue(record, kABPersonNicknameProperty, (__bridge CFTypeRef)(phone), anError);
    ABRecordSetValue(record, kABPersonOrganizationProperty, (__bridge CFTypeRef)(@"jiafenzhushou"), anError);

    
    [phonelist enumerateObjectsUsingBlock:^(NSDictionary *phone, NSUInteger idx, BOOL *stop) {
